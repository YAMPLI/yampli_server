...
const test1 = [
  ...
  '중간에 추천하면? $zerobaseone, 인불름, 서바이벌 1위한 애들이래,최신곡 오후 6:02',
  '$$ baek yerin ,스퀘어,이거 가을날씨에 듣기 딱임, 감성',
  '얌플리데이터\\',
  '생성중',
  '$$ IU,라일락, 이거 좋더라',
  '생성',
  '$$infinite, 내꺼하자, 얘네 컴백했더라, 댄스 오후 6:14',
  '중간에 추천하면? $$zb1, inbloome, 서바이벌 1위한 애들이래,최신곡 오후 6:15',
  '2023년 8월 3일 목요일',
  '$$ghost9 , Fever, 피크타임이래',
  '$$배너 , performer, 피크타임이래',
...
const test2 = ['$$ghost9 , Fever, 피크타임이래'];
...
const spotifyApi = async (artist, title) => {
  ...
  // 검색 결과 undefined인 경우 에러 처리
  const query = `"${artist}" "${title}"`;
  // console.log(await findVideo(artist, title));

  // const query = await findVideo(artist, title);
  const params = {
    q: query,
    type: 'track',
    limit: 1,
  ...
  try {
    ...
    if (response.data.tracks.items.length > 0) {
      return response.data.tracks.items[0].album;
